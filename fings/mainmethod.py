...
# Copyright 2018-2024 CP Wardell
#
...
def main():
    ...
    parser.add_argument("--noplots", help="Don't produce plots", required=False, default=False, action='store_true')
    ...
    if(args.ICGC):
        args.p="icgc_filter_parameters.txt"
    ...
    if(args.logging=="INFO"):
        logging.basicConfig(level=logging.INFO,format='%(asctime)s %(levelname)s %(message)s',datefmt='%Y-%m-%d %H:%M:%S',filename=args.d+'/log.txt',filemode='w')
    ...
    if(args.logging=="DEBUG"):
        logging.basicConfig(level=logging.DEBUG,format='%(asctime)s %(levelname)s %(message)s',datefmt='%Y-%m-%d %H:%M:%S',filename=args.d+'/log.txt',filemode='w')
        console = logging.StreamHandler()
    ...
    logging.info("VCF is: "+str(args.v))
    logging.info("Tumor BAM is: "+str(args.t))
    logging.info("Normal BAM is: "+str(args.n))
    logging.info("Output directory is: "+str(args.d))
    logging.info("Filtering parameter file is: "+str(args.p))
    logging.info("Reference genome file is: "+str(args.r))
    logging.info("Number of records per chunk is: "+str(args.c))
    logging.info("Maximum read depth is: "+str(args.m))
    logging.info("Processor threads used is: "+str(args.j))
    ...
    if(args.p=="filter_parameters.txt" or args.ICGC):
        codedirectory = os.path.dirname(os.path.realpath(__file__))
        args.p = os.path.join(codedirectory,args.p)
    ## Read filtering parameters into a dictionary
    logging.info("Reading filtering parameters from this file: "+args.p)    
    pdict={}
    try:
        with open(args.p) as tsvin:
    ...
    tdata=args.d+"/tumor.combined.txt.gz"
    ndata=args.d+"/normal.combined.txt.gz"
    sdata=args.d+"/summarystats.txt.gz"
    ...
    if(args.overwrite or (not os.path.exists(ndata) and not os.path.exists(tdata))):
        ...
        try:
            nvcflines=vcfcount(args.v)
        except Exception as e:
            print("CRITICAL ERROR: unreadable VCF file: "+str(e))
            sys.exit()
        maxchunks=math.ceil(nvcflines/args.c)
        ...
        tumvars=Parallel(n_jobs=args.j, backend="threading")(delayed(primary)(vcfchunk,"tumor",chunknumber,maxchunks,args) for chunknumber,vcfchunk in enumerate(vcfyield(vcf_reader,args.c,nvcflines)))

        ## Normal bam
        vcf_reader = vcfpy.Reader.from_path(args.v)
        normvars=Parallel(n_jobs=args.j, backend="threading")(delayed(primary)(vcfchunk,"normal",chunknumber,maxchunks,args) for chunknumber,vcfchunk in enumerate(vcfyield(vcf_reader,args.c,nvcflines)))
    ...
    ## Filter data
    applyfilters(tdata,ndata,sdata,pdict,args)
